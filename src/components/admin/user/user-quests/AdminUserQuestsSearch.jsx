...
import { useAdminUserQuestsQuery, useAdminUserQuestDelete } from '@hooks/admin/user-quests'
...
const AdminUserQuestsSearch = () => {
  ...
  return (
    ...
    >
      ...
      <QuestsOfUserTable userQuests={userQuests} userQuestsSet={userQuestsSet} />
    </Flex>
  )
}
...
const QuestsOfUserTable = ({ userQuests, userQuestsSet }) => {
          ...
          <Tbody>
            {userQuests &&
              ...
              userQuests.map((userQuest, index) => {
                const { quest, rewardType, rewardedQty, updatedAt, id, userId, questId } = userQuest
                    ...
                    <Td>
                      <Icon
                        ...
                        onClick={async () => {
                          ...
                          let deleteOp = await deleteUserQuestsAsync(payload)

                          if (!deleteOp.isError) {
                            let currentUserQuests = userQuests.filter((q) => q.id !== id)
