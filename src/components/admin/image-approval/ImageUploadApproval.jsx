...
const ImageUploadApproval = () => {
  ...
  //   try {
  //     setIsWorking(true)
  //     let res = await axios.post(`${Enums.BASEPATH}/api/admin/quest/hide-userquest`, userQuest)
  ...
  //       </span>{' '}
  //       of Total: <span className="font-semibold text-secondary 900 dark:text-white">{count}</span>{' '}
  //     </span>
  ...
  //                 </a>
  //                 <div className="text-sm break-words">Wallet: {user?.wallet}</div>
  //                 <div className="text-sm mt-2 ">Discord: {user?.discordUserDiscriminator}</div>

  //                 {extendedUserQuestData &&
  //                   extendedUserQuestData?.messageId &&
  //                   extendedUserQuestData.messageId.length > 0 && (
  //                     <label className=" text-green-500">Submitted</label>
  //                   )}
  //                 {extendedUserQuestData && !extendedUserQuestData?.messageId && (
  //                   <>
  //                     <button
