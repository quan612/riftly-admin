    ...
    case 'POST':
      try {
        ...
        if (type === Enums.DISCORD) {
          ...
          if (discordBotToken.trim().length < 1) {
            return res.status(200).json({
              isError: true,
              message: 'Missing config for querying discord',
            })
          }
        ...
        if (type === Enums.TWITTER) {
          if (twitterBearerToken.trim().length < 1) {
            return res.status(200).json({
              isError: true,
              message: 'Missing config for querying twitter',
            })
          }
          ...
          if (id && username) {
            ...
            return res.status(200).json({
              isError: true,
              message: `Cannot resolve Twitter Handle ${user} to TwitterId`,
            })
          }
        }
        if (type === Enums.WALLET) {
          ...
          if (existingUser) {
            return res
              .status(200)
              .json({ isError: true, message: `${wallet} existed on database.` })
          } else {
            const newUser = await prisma.whiteList.create({
              data: {
                wallet,
