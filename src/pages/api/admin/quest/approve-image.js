    ...
    case 'POST':
      try {
        ...
        if (discordSecret.trim().length < 1 || discordBackend.trim().length < 1) {
          return res.status(200).json({ isError: true, message: 'Missing Discord Server Config!' })
        }
        ...
        if (!entry) {
          return res.status(200).json({ isError: true, message: 'Cannot find this user quest!' })
        }
        ...
        if (!discordMsg?.data?.response?.id) {
          return res.status(200).json({
            isError: true,
            message: 'Cannot get discord message id after uploaded. Pls contact administrator!',
          })
        }
