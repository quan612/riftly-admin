  ...
  switch (method) {
    ...
    case 'POST':
      try {
        ...
        if (!questType) {
          return res.status(200).json({ isError: true, message: `Cannot find quest type ${type}` })
        }
        ...
        if (id === 0) {
          ...
          let existingDiscordTwitterAuth = discordTwitterAuthCheck(existingQuests, questType.name)
          ...
          let existingCodeQuest = codeQuestCheck(existingQuests, extendedQuestData, questType.name)
          ...
          await axios
            .post(`${questVariable.hostUrl}/api/admin/web-push/push-notification`, payload)
            .catch((err) => {
              console.log(err)
            })
        } else {
...
const joinDiscordCheck = (existingQuests, extendedQuestData, type) => {
...
const retweetCheck = (existingQuests, extendedQuestData, type) => {
  ...
  return twitterRetweetQuest.some((q) => q.extendedQuestData.tweetId === extendedQuestData.tweetId)
}
...
const imageUploadQuestCheck = (existingQuests, extendedQuestData, type) => {
  ...
  return existingCodeQuests.some(
    (q) =>
      q.extendedQuestData.eventName.toLowerCase() === extendedQuestData.eventName.toLowerCase() ||
